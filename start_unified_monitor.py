...
def show_banner():
    ...
    print("=" * 70)
    print("🚀 跨平台性能监控工具 - 统一启动器 v1.3.0")
    print("=" * 70)
    print(f"💻 当前系统: {current_os}")
    print("✨ 功能特性:")
    print("  • 同时支持iOS和Android设备监控")
    if current_os == 'Windows':
        print("  • ⚠️  Windows 上 iOS 监控为实验性功能（需安装 iTunes）")
...
def detect_devices():
    ...
    status_text = ""
    if current_os == 'Windows' and not ios_connected:
        status_text = " (需安装 iTunes)"
    
    print(f"  {'✅' if ios_connected else '❌'} iOS设备: {'已连接' if ios_connected else '未连接' + status_text}")
...
def interactive_mode():
    ...
    if current_os == 'Windows':
        print("\n  ⚠️  提示: Windows 上 iOS 监控需要安装 iTunes 或 Apple Device Support")
    ...
    if choice == '1':
        ...
        print("\n🍎 启动iOS监控服务...")
        if current_os == 'Windows':
            print("⚠️  Windows 上使用 iOS 监控为实验性功能")
            print("💡 确保已安装 iTunes 或 Apple Device Support")
        start_ios_monitor()
